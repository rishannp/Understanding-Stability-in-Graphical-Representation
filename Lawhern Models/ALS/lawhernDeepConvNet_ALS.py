...
from EEGModels import DeepConvNet  # Ensure you have this model

# Data directory and subject info
...
for subject_number in subject_numbers:
    ...
    for key in idx:
        for i in range(S1.shape[1]):
            S1[key][0, i] = bandpass(S1[key][0, i], [8, 30], fs)  # Bandpass filtering
    ...
    # Split data into 4 sessions (no shuffling)
    session_size = len(data) // 4
    sessions = [data[i * session_size:(i + 1) * session_size] for i in range(4)]
    label_sessions = [labels[i * session_size:(i + 1) * session_size] for i in range(4)]

    # Train on session 1, test on sessions 2, 3, and 4
    train_data, train_labels = sessions[0], label_sessions[0]
    test_sessions = sessions[1:]
    test_labels_sessions = label_sessions[1:]
