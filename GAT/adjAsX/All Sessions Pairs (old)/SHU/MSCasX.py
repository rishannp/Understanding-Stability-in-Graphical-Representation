...
# Function to compute MSC matrix for a single trial
def mscfcn(eegData, fs=250, nperseg=250):
    ...
    for electrode1 in range(numElectrodes):
        for electrode2 in range(electrode1 + 1, numElectrodes):
            # Compute coherence between electrode signals
            f, Cxy = coherence(eegData[electrode1, :], eegData[electrode2, :], fs=fs, nperseg=nperseg)
            msc_value = np.mean(Cxy)  # Mean MSC over frequencies
            mscMatrix[electrode1, electrode2] = msc_value
            mscMatrix[electrode2, electrode1] = msc_value
    
    return mscMatrix


# Function to create graphs and compute adjacency matrices for MSC matrices
def create_graphs_and_edges(msc_matrices, threshold):
    graphs = []
    adj_matrices = np.zeros([msc_matrices.shape[0], msc_matrices.shape[0], msc_matrices.shape[2]])  # (Electrodes, Electrodes, Trials)
    edge_indices = []
    
    for i in range(msc_matrices.shape[2]):
        G = nx.Graph()
        G.add_nodes_from(range(msc_matrices.shape[0]))  # Nodes represent electrodes
        ...
        for u in range(msc_matrices.shape[0]):
            for v in range(u + 1, msc_matrices.shape[0]):  # Avoid duplicate edges
                if msc_matrices[u, v, i] > threshold:
                    # Add edge to graph
                    G.add_edge(u, v, weight=msc_matrices[u, v, i])
                    
                    # Store the edge in adjacency matrix (symmetric)
                    adj_matrices[u, v, i] = msc_matrices[u, v, i]
                    adj_matrices[v, u, i] = msc_matrices[u, v, i]
...
# Function to compute MSC matrices for all trials in all sessions of a subject
def compute_msc(subject_data, fs=250, nperseg=250):
    session_msc_data = {}  # To store MSC matrices, labels, and graphs for each session
    ...
    for session_id, session_data in subject_data.items():
        ...
        msc_matrices = np.zeros((numElectrodes, numElectrodes, numTrials))
        
        # Compute MSC for each trial in the session
        for trial_idx in range(numTrials):
            eeg_trial_data = data[trial_idx]  # Shape: (Channels, TimeSteps)
            msc_matrices[:, :, trial_idx] = mscfcn(eeg_trial_data, fs=fs, nperseg=nperseg)
        
        # Convert labels to torch tensor
        label_tensor = torch.tensor(labels, dtype=torch.long)
        
        # Create graphs, adjacency matrices, and edge indices
        adj_matrices, edge_indices, graphs = create_graphs_and_edges(msc_matrices, threshold=0)
        
        # Store session-level data for MSC matrices, labels, and graphs
        session_msc_data[session_id] = {
            'msc_matrices': msc_matrices,   # Shape: (Electrodes, Electrodes, Trials)
...
for subject_number in subject_msc_data:
    all_session_data = subject_msc_data[subject_number]  # Data for all sessions of the subject
    ...
    for train_session_idx in range(len(all_session_data)):
        ...
        traind = []
        for i in range(np.size(traindata['msc_matrices'],2)):
        ...
        for test_session_idx in range(len(all_session_data)):
            ...
            testd = []
            for i in range(np.size(testdata['msc_matrices'],2)):
