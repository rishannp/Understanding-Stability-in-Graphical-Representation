...
def bandpasscfc(data, freq_range, sample_rate=250, order=5):
...
# Function to compute CFC matrix for a single trial
def cfcfcn(eegData, fs=250):
    ...
    beta_amplitude = np.abs(hilbert(beta_data, axis=1))
...
# Function to create graphs and compute adjacency matrices for CFC matrices
def create_graphs_and_edges(cfc_matrices, threshold=0):
    graphs = []
    adj_matrices = np.zeros([cfc_matrices.shape[0], cfc_matrices.shape[0], cfc_matrices.shape[2]])  # (Electrodes, Electrodes, Trials)
    edge_indices = []
    
    for i in range(cfc_matrices.shape[2]):
        G = nx.Graph()
        G.add_nodes_from(range(cfc_matrices.shape[0]))  # Nodes represent electrodes
        ...
        for u in range(cfc_matrices.shape[0]):
            for v in range(u + 1, cfc_matrices.shape[0]):  # Avoid duplicate edges
                if cfc_matrices[u, v, i] > threshold:
                    # Add edge to graph
                    G.add_edge(u, v, weight=cfc_matrices[u, v, i])
                    
                    # Store the edge in adjacency matrix (symmetric)
                    adj_matrices[u, v, i] = cfc_matrices[u, v, i]
                    adj_matrices[v, u, i] = cfc_matrices[u, v, i]
...
# Function to compute CFC matrices for all trials in all sessions of a subject
def compute_cfc(subject_data, fs=256):
    session_cfc_data = {}  # To store CFC matrices, labels, and graphs for each session
    ...
    for session_id, session_data in subject_data.items():
        ...
        cfc_matrices = np.zeros((numElectrodes, numElectrodes, numTrials))
        
        # Compute CFC for each trial in the session
        for trial_idx in range(numTrials):
            eeg_trial_data = data[trial_idx]  # Shape: (Channels, TimeSteps)
            cfc_matrices[:, :, trial_idx] = cfcfcn(eeg_trial_data, fs=fs)
        
        # Convert labels to torch tensor
        label_tensor = torch.tensor(labels, dtype=torch.long)
        
        # Create graphs, adjacency matrices, and edge indices
        adj_matrices, edge_indices, graphs = create_graphs_and_edges(cfc_matrices, threshold=0)
        
        # Store session-level data for CFC matrices, labels, and graphs
        session_cfc_data[session_id] = {
            'cfc_matrices': cfc_matrices,   # Shape: (Electrodes, Electrodes, Trials)
    ...
    return session_cfc_data


# Dictionary to store CFC data for all subjects
subject_cfc_data = {}
...
for subject_id, subject_data in merged_fold_data.items():
    ...
    subject_cfc_data[subject_id] = {}
    
    # Compute CFC matrices, labels, and graph-related data for each session of the subject
    session_data = compute_cfc(subject_data)
    
    # Store the computed data for each session in the subject's data dictionary
    subject_cfc_data[subject_id] = session_data
...
for subject_number in subject_cfc_data:
    all_session_data = subject_cfc_data[subject_number]  # Data for all sessions of the subject
    ...
    for train_session_idx in range(len(all_session_data)):
        ...
        traind = []
        for i in range(np.size(traindata['cfc_matrices'],2)):
        ...
        for test_session_idx in range(len(all_session_data)):
            ...
            testd = []
            for i in range(np.size(testdata['cfc_matrices'],2)):
