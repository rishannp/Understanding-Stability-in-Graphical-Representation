...
def cmifcn(eegData):
    # Restrict to the first 19 electrodes
    eegData = eegData[:, :19]
    ...
    for electrode1 in range(numElectrodes):
        for electrode2 in range(electrode1 + 1, numElectrodes):
            ...
            # Extract scalar value from array and store the MI value in the matrix (symmetrically)
            cmiMatrix[electrode1, electrode2] = cmi_value.item()  # Fix applied here
            cmiMatrix[electrode2, electrode1] = cmi_value.item()  # Fix applied here
...
def compute_cmi(subject_data):
    ...
    # Use only the first 19 electrodes for CMI computation
    numElectrodes = 19  # Limit to the first 19 electrodes
    numTrials = subject_data['L'].shape[1]
    cmi = {field: np.zeros((numElectrodes, numElectrodes, numTrials)) for field in idx}
...
save_dir = r"C:\Users\uceerjp\Desktop\PhD\Year 2\DeepLearning-on-ALS-MI-Data\Graphs\Understanding Stability in Graphical Representation\GAT\adjAsX\Session 1 v X Pairs\ALS\Explain\CMI"
