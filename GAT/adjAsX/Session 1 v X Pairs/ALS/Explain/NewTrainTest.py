...
for subject_number in subject_numbers:
    ...
    # Track best epoch based on average accuracy and stability
    best_epoch = 0
    best_avg_accuracy = 0
    best_stability = float('inf')
    optimal_fold_acc = 0  # Highest accuracy for each fold
    fold_test_accuracies = []
    fold_weights = {}
    fold_att = {}
    
    # Store training and inference times
    train_times = []
    inference_times = []
    ...
    for train_fold_idx in range(num_folds):
        ...
        # Create training data (only one fold used for training)
        ...
        # Define the GAT model class (already defined above)
        ...
        for epoch in range(1, 501):  # Train for 500 epochs
            ...
            for test_fold_idx in range(num_folds):
                ...
                # Store the test accuracy for this session
                session_accuracies.append(test_acc)
    
            # Calculate average accuracy and stability (variance across sessions)
            avg_accuracy = np.mean(session_accuracies)
            stability = np.std(session_accuracies)
    
            # Update best epoch based on average accuracy and stability
            if avg_accuracy > best_avg_accuracy or (avg_accuracy == best_avg_accuracy and stability < best_stability):
            ...
            # Store fold accuracies for reporting after training
            fold_test_accuracies.append(session_accuracies)
    
        # After training, save the results for the best epoch
        fold_name = f"{train_fold_idx + 1}v{test_fold_idx + 1}"
        fold_accuracies = {
            'fold_name': fold_name,
            'best_epoch': best_epoch,
            'best_accuracy': session_accuracies,
        ...
        print(f"Fold {fold_name}: Best Epoch: {best_epoch}, Best Avg Accuracy: {best_avg_accuracy:.4f}, Stability: {best_stability:.4f}")
...
base_save_path = r"C:\Users\uceerjp\Desktop\PhD\Year 2\DeepLearning-on-ALS-MI-Data\Graphs\Best_Weights"
...
for subject, train_test_pairs in all_subjects_accuracies.items():
    for pair_idx, pair_data in enumerate(train_test_pairs):
        ...
        pca_folder = os.path.join(fold_save_path, "PCA_Plots")
        ...
        plt.ylabel("Trials (Left/Right)", fontsize=12)
