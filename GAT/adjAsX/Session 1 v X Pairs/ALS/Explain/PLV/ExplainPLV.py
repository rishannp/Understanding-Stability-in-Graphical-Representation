...
def plvfcn(eegData):
    ...
    eegData = eegData[:, :19]  # Slice to include only the first 19 electrodes
...
def compute_plv(subject_data):
    idx = ['L', 'R']
    numElectrodes = 19  # Restrict computations to the first 19 electrodes
    plv = {field: np.zeros(
    ...
    for i, field in enumerate(idx):
        for j in range(subject_data.shape[1]):
            x = subject_data[field][0, j][:, :19]  # Slice to first 19 electrodes
...
save_dir = r"C:\Users\uceerjp\Desktop\PhD\Year 2\DeepLearning-on-ALS-MI-Data\Graphs\Understanding Stability in Graphical Representation\GAT\adjAsX\Session 1 v X Pairs\ALS\Explain\PLV"
