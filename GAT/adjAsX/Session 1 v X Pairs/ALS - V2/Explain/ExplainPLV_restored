        ...
        for test_fold_idx in range(num_folds):
            ...
            class GAT(nn.Module):
                ...
                def forward(self, x, edge_index, batch, return_attention_weights=True):
                    ...
                    w1, att1 = self.conv1(x, edge_index, return_attention_weights=True)
                    w1 = F.relu(w1)
                    w1 = self.gn1(w1, batch)  # Apply GraphNorm
    
                    # Apply second GAT layer and normalization
                    w2, att2 = self.conv2(w1, edge_index, return_attention_weights=True)
                    w2 = F.relu(w2)
                    w2 = self.gn2(w2, batch)  # Apply GraphNorm
    
                    # Apply third GAT layer and normalization
                    w3, att3 = self.conv3(w2, edge_index, return_attention_weights=True)
                    ...
                    return o, w1,w2,w3, att1, att2, att3
            ...
            def train():
                ...
                for data in train_loader:
                    out,w1,w2,w3,att1,att2,att3 = model(data.x, data.edge_index, data.batch)
                    loss = criterion(out, data.y)
                    loss.backward()
                    optimizer.step()
                    optimizer.zero_grad()
                end_time = time.time()  # End timing
                return end_time - start_time, w1,w2,w3, att1, att2, att3  # Return elapsed time
            ...
            def test(loader):
                ...
                return correct / len(loader.dataset), inference_time, w1,w2,w3, att1, att2, att3  # Return accuracy and inference time
            ...
            for epoch in range(1, 501):  # Note: Range is 1 to 500 (inclusive)
                ...
                epoch_train_time, trainw1,trainw2,trainw3, trainatt1,trainatt2,trainatt3 = train()
                train_times.append(epoch_train_time)
                
                # Get train and test accuracy, along with inference time
                # train_acc, _ = test(train_loader)
                test_acc, inference_time, testw1,testw2,testw3,testatt1,testatt2,testatt3 = test(test_loader)
                ...
                if test_acc > optimal_fold_acc:  # Compare accuracy only (not tuple)
                    ...
                    print(optimal_fold_acc)
                    ### NEED TO STORE Weights and Attention only for best model ###
                    
                    # Store model weights and attention weights
                    best_weights = {
                        'w1': testw1.detach().cpu().numpy(),
                        'w2': testw2.detach().cpu().numpy(),
                        'w3': testw3.detach().cpu().numpy(),
                    }
                    best_att = {
                        'att1': testatt1[1].detach().cpu().numpy(),
                        'att2': testatt2[1].detach().cpu().numpy(),
                        'att3': testatt3[1].detach().cpu().numpy(),
                    }
                    
                    # Save the model weights
                    torch.save(model.state_dict(), f"best_model_fold_{train_fold_idx + 1}v{test_fold_idx + 1}.pth")
