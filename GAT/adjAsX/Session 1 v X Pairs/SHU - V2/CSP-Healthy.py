...
for subject, data in filtered_data_split.items():
    ...
    # We will only use fold 1 as the training fold and test on folds 2, 3, 4, 5
    fold_names = list(prepared_folds.keys())
    
    # Only use the first fold for training
    train_fold_key = fold_names[0]  # Fold 1 (Training)
    test_fold_keys = fold_names[1:]  # Folds 2, 3, 4, 5 (Testing)
    
    # Store results for this subject
    accuracies[subject] = {}
    
    # Iterate over the test folds (2, 3, 4, 5)
    for test_fold_key in test_fold_keys:
        ...
        # Print subject and pair info with accuracy
        print(f'Subject: {subject}, Training Fold: {train_fold_key}, Testing Fold: {test_fold_key}, Accuracy: {accuracy:.4f}')
...
heatmap_data_accuracy = df.pivot(index='Subject', columns='Train-Test Pair', values='Accuracy')

plt.figure(figsize=(14, 8))
sns.heatmap(heatmap_data_accuracy, annot=True, cmap="YlGnBu", fmt=".2f")
...
# Iterate through the accuracies dictionary and print fold accuracies for each subject
for subject, pairs in accuracies.items():
    print(f"Subject: {subject}")
    
    # Iterate through the fold pairs and print the accuracy
    for pair, accuracy in pairs.items():
        # Format the fold pair for output (e.g., '1v2', '1v3', '1v4', '1v5')
        print(f"{pair}: {accuracy:.4f}")
    
    # Print a newline after each subject for readability
    print()
