...
def mscfcn(eegData, fs=256, nperseg=256):
    ...
    for electrode1 in range(numElectrodes):
        for electrode2 in range(electrode1 + 1, numElectrodes):
            ...
            mscMatrix[electrode1, electrode2] = np.mean(Cxy)
            mscMatrix[electrode2, electrode1] = np.mean(Cxy)
    
    return mscMatrix

def compute_msc(subject_data, fs=256, nperseg=256):
    idx = ['L', 'R']
    numElectrodes = subject_data['L'][0,1].shape[1]
    msc = {field: np.zeros((numElectrodes, numElectrodes, subject_data.shape[1])) for field in idx}

    for i, field in enumerate(idx):
        for j in range(subject_data.shape[1]):
            x = subject_data[field][0, j]
            msc[field][:, :, j] = mscfcn(x, fs=fs, nperseg=nperseg)
...
def create_graphs(msc, threshold):
    graphs = []
    for i in range(msc.shape[2]):
        G = nx.Graph()
        G.add_nodes_from(range(msc.shape[0]))
        for u in range(msc.shape[0]):
            for v in range(msc.shape[0]):
                if u != v and msc[u, v, i] > threshold:
                    G.add_edge(u, v, weight=msc[u, v, i])
        graphs.append(G)
    return graphs
...
mat_fname = pjoin(data_dir, f'OGFS{1}.mat')
mat_contents = sio.loadmat(mat_fname)
subject_data[f'S{1}'] = mat_contents[f'Subject{1}']
S1 = subject_data[f'S{1}'][:,:]
msc, y = compute_msc(S1)
#%%
...
for subject_number in subject_numbers:
    ...
    msc, y = compute_msc(S1)
    threshold = 0.1
    graphs = create_graphs(msc, threshold)
    numElectrodes = S1['L'][0,1].shape[1]
...
with open('cross_session_accuracies.pkl', 'rb') as file:
    all_subjects_accuracies = pd.read_pickle(file)
...
for subject, folds in all_subjects_accuracies.items():
    for fold_data in folds:
        ...
        accuracy = fold_data['optimal']  # Using 'optimal' accuracy here
        data.append({
            'Subject': subject,
...
# Create boxplot for accuracy distribution across subjects
...
# Calculate and print average accuracies for each subject
average_stats = {}
...
# Print average statistics for each subject
