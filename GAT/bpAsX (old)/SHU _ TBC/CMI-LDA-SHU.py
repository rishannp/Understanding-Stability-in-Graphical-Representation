...
for subject in split_results.keys():
    ...
    # Initialize a matrix to store accuracies (num_folds x num_folds)
    fold_accuracies = np.zeros((num_folds, num_folds))

    # Loop through all combinations of train and test folds
    for train_fold_idx in range(num_folds):
        ...
        # Train the LDA model on the current training fold
        lda.fit(x_train_reshaped, y_train)

        # Test on all other folds
        for test_fold_idx in range(num_folds):
            if train_fold_idx != test_fold_idx:
                ...
                # Calculate accuracy for the current test fold
                accuracy = accuracy_score(y_test, y_pred)
                fold_accuracies[train_fold_idx, test_fold_idx] = accuracy
                print(f"Accuracy (train on fold {train_fold_idx + 1}, test on fold {test_fold_idx + 1}): {accuracy:.4f}")

    # Print the pairwise accuracy matrix for the subject
    print(f"Pairwise fold accuracies for Subject {subject}:")
